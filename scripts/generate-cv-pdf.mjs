#!/usr/bin/env node
/**
 * Generate public/cv-stephane-karasiewicz.pdf from /cv using Playwright.
 *
...
const PORT = 14321;
const previewUrl = `http://localhost:${PORT}/cv`;
const outputPath = path.join(projectRoot, 'public', 'cv-stephane-karasiewicz.pdf');
...
async function main() {
    ...
    try {
        ...
        if (preview.pid && !preview.killed) {
            ...
            // Give the process a brief moment, then escalate.
            await sleep(500);
